    ...
    for (clust_iter in c){
      ...
      for (fold_iter in k){
        ...
        while (Top_features_selected == FALSE){
          ...
          if ( (vector_AUC[COUNTER] >= vector_means_AUC_neg_ctrl[COUNTER]) && (vector_p_vals[COUNTER] < p_value_threshold) ){
            COUNTER = COUNTER + 1  
          } else{
            if (COUNTER > 2){
...
test_sig_MRs = MRs_per_fold(Risk_group_classification_object = val_test, k = 1:5, c = 1, pipeline_iter = 1, p_value_threshold = 0.05)
...
Internal_validation <- function(Risk_group_classification_object = NULL,
  ...
  if (loadfile == TRUE){
    ...
    Risk_group_classification_object$Experimental_Settings$interactome = interactome
  }
...
test_internal_validation <- Internal_validation(Risk_group_classification_object = val_test, 
                                                pipeline_iter = 1, nb_folds = 5, 
                                                c = 1, 
                                                classification_algorithm = 'svm', 
                                                nb_iterations = 10, 
                                                interactome = pruneRegulon(network_coTF_TF), 
                                                mRNA_control = T, savefile = F)

vp_test <- viper(test_internal_validation, regulon = pruneRegulon(network_coTF_TF), method = 'none', verbose = T)
...
val_test$iter_1$folds$GEM <- norm_res$iter_1$folds$GEM
val_test$iter_1$patient_group_1 <- norm_res$iter_1$patient_group_1
val_test$iter_1$patient_group_2 <- norm_res$iter_1$patient_group_2
val_test$iter_1$heldout_patients_group_1 <- norm_res$iter_1$heldout_patients_group_1
val_test$iter_1$heldout_patients_group_2 <- norm_res$iter_1$heldout_patients_group_2
