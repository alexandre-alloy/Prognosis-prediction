...
Patient_groupings <- function(samples, 
  ...
  if ((SMOTE == TRUE) && (is.null(GEM) == TRUE)) stop('ERROR: With SMOTE == TRUE, need to supply a GEM')
  ...
  if ((equilibrate_classes == TRUE) && (SMOTE == FALSE)){
    ...
    for (i in 1:nb_iterations){
      if (overlapping == TRUE){
        list_validation_group_1[[i]] = sample(x = samples_group_1, size = validation_fold_size, replace = T)
        list_training_group_1[[i]] = sample(x = setdiff(samples_group_1, list_validation_group_1[[i]]), size = nb_samples_to_equilibrate_to - validation_fold_size, replace = T)
        list_validation_group_2[[i]] = sample(x = samples_group_2, size = validation_fold_size, replace = T)
        list_training_group_2[[i]] = sample(x = setdiff(samples_group_2, list_validation_group_2[[i]]), size = nb_samples_to_equilibrate_to - validation_fold_size, replace = T)
      }
...
####
#test#
a <- Patient_groupings(samples = colnames(GEM), 
                       samples_group_1 = dead, 
                       samples_group_2 =  alive, 
                       nb_iterations = 5, 
                       validation_fold_size = round(0.1*ncol(GEM)), 
                       overlapping = T, 
                       equilibrate_classes = T, 
                       subsample_majority_class = T, 
                       oversample_minority_class = F)


sapply(1:5, function(i) intersect(a$list_training[[i]], a$list_validation[[i]]))

sapply(1:5, function(i) length(which(a$list_training[[i]] %in% alive)))
sapply(1:5, function(i) length(which(a$list_training[[i]] %in% dead)))
sapply(1:5, function(i) length(which(a$list_validation[[i]] %in% alive)))
sapply(1:5, function(i) length(which(a$list_validation[[i]] %in% dead)))

sapply(1:5, function(j) sapply(1:5, function(i) length(intersect(a$list_training[[j]], a$list_training[[i]]))))
