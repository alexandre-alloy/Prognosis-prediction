...
Initial_processing_folds <- function(patient_group_1, patient_group_2,
                                    CV_type = c('kfold', 'LOOCV', 'monte carlo'), 
                                    nb_folds = 5, validation_fold_size = NULL, 
  ...
  for (pipeline_iterations in 1:nb_iterations_pipeline){
    ...
    if ( ( (CV_type == 'kfold') | (CV_type == 'LOOCV') | (CV_type == 'monte carlo') ) == FALSE){
      stop('ERROR: CV_type incorrectly set. Must be either kfold, LOOCV or monte carlo')
    }
    
    if (CV_type == 'kfold') overlapping = FALSE
    if (CV_type == 'monte carlo') overlapping = TRUE
    if (CV_type == 'LOOCV'){
  ...
  if (savefile == TRUE) saveRDS(object = list_folds, file = paste0(save_path, '/folds.rda'))
...
Initial_normalization_clust <- function(Initial_processing_folds_object, #object name resulting from Initial_processing_folds
  ...
  for (pipeline_iterations in pipeline_iter){
    ...
    #lists containing the vst-normalized matrices
    ...
    for (fold_iter in compute_folds){
      ...
      vps_training = viperSignature(GEM_training, GEM_training, per = 500, method = 'zscore', verbose = F)
      if (mRNA_control == FALSE) vp_training = viper(eset = vps_training, regulon = interactome, method = 'none', verbose = F)
